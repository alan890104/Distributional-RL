...
class Gridworld_RandReward_5x5_Env(gym.Env):
    ...
    def reset(self):
        # Reset the state uniformly at random
        #self.obs = 6
        self.obs = np.random.randint(1, self.observation_space.n, size=1)  # origin format
        # self.obs = np.random.choice([1,2,5,6,7,10,11,12],size=1)
        #self.obs = np.array([24])

        #### for the uniform search ####
        # self.obs = np.array([self.reset_order])
        # self.reset_order += 1
        # if self.reset_order == 25:
        #     self.reset_order = 1
