...
class Network(nn.Module):
    def __init__(self, num_state, num_action, num_quantile):
        ...
        ### hidden layer is default 512
        self.input_layer = nn.Linear(num_state, 512)
        self.layer_output = nn.Linear(512, num_action * num_quantile)
...
class QR_DQN():
    def __init__(self, env, num_quantile, learning_rate):
        ...
        self.Z = Network(num_state=env.observation_space.shape[0], num_action=env.action_space.n, num_quantile=num_quantile)
        self.Z_target = Network(num_state=env.observation_space.shape[0], num_action=env.action_space.n, num_quantile=num_quantile)
        ...
        self.eps = 0.05
        self.discount = 0.95
        self.replaymemory = replaymemory.replaymemory(10000, env.observation_space.shape[0]) # replayermemory(size, state space)
        self.n_iter = 1000
